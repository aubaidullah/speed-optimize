...
const HotelByDestination = dynamic(() =>
  import("@/components/hotel/by-destination"),
);
...
const Hotels = ({ data, meta }) => {
  ...
  useEffect(() => {
    ...
    setCheckindate({
      day: parseInt(minDate["day"]),
      month: parseInt(minDate["month"]),
      year: parseInt(minDate["year"]),
    });
  ...
  const Search = async () => {
    setLoading(true);
    const result = await axios.post(Constants.api + "/api/v1/geo/cities/", {
      av: "",
      id: "",
      pt: "",
      text: searchkey,
...
export async function getServerSideProps(context) {
  ...
  return {
    props: { data: res.data.hotels.output, meta: meta.data.meta.output.tags },
  };
}
