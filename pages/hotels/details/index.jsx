...
const HotelDetail = ({ hotel, meta }) => {
  ...
  const bread = {
    ...
    enabled: [
      ...
      },
      {
        ...
        href: `/hotels/hotel-in-${strToUrl(hotel.hotel.cityname)}-${
          hotel.hotel.cid
        }`,
      },
    ],
  ...
  useEffect(() => {
    updateHotel({
      ...
      travellers: selectedHotel.travellers,
    });
  }, [selectedRoom]);
                        ...
                        <li className="slide">
                          {hotel.hotel.images ? (
                            <Image
                              ...
                              src={jpgToWebp({
                                uri: hotel.hotel.images.split(",")[0],
                              })}
                              layout="fill"
                            />
                          ) : (
                                ...
                                {index == 1 ? (
                                  ...
                                  >
                                    <button className="btn_listing bg-white normal-case">
                                      Show all{" "}
                                      {hotel.hotel.images.split(",").length}{" "}
                                      Photos
                                    </button>
                                  </div>
                                ) : (
...
export async function getServerSideProps(context) {
  ...
  const metas = {
    ...
    longDesc: meta.data.meta.output.tags.longDesc
      .replace(/<HOTEL>/g, hotelname)
      .replace(/<CITY>/g, cityname),
    keywords: meta.data.meta.output.tags.keywords
      .replace(/<HOTEL>/g, hotelname)
      .replace(/<CITY>/g, cityname),
  };
