...
const Login = (props) => {
    ...
    if (isNaN(user.username * 1)) {
      ...
      if (validator.current.allValid()) {
        const response = await axios.post(
          Constants.api + "/api/v1/user/login",
          {
            av: Constants.av,
            pt: Constants.pt,
            username: user.username,
  ...
  const handleOTPSubmit = async (e) => {
    ...
    if (otpvalidator.current.allValid()) {
      const response = await axios.post(
        Constants.api + "/api/v1/user/verifyotp",
        {
          av: Constants.av,
          pt: Constants.pt,
          username: user.username,
  ...
  const handleForgetCredentials = (e) =>
    setForgetCredentials({
      ...forgetCredentials,
          ...
          <div className={tw`m-3`}>
            ...
            <form onSubmit={handleOTPSubmit}>
              <div className="form-group">
                ...
                />
                <div className="Invalid_num">
                  {otpvalidator.current.message("otp", loginOTP, "required")}
                </div>
              </div>
              <div className="form-group">
