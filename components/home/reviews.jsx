  ...
  const SReview = ({item,modal=false}) => {
    ...
    return <>
        ...
        <div className="text-comment">
          {
            modal==false?          <p>{review.substring(0, 150)}
            {
              review.length>150 ?
              <span className='anchor_link pl-2 cursor-pointer' onClick={()=>{setRV(item), setIsshow(true)}}>read more</span>:""
      ...
      return (
        <div key={j}>
          <div className={`col-sm-12 col-xs-12 _cr_mb px-4`} key={j}>
            <div className={tw`comment_box hover:shadow-lg border border-gray-200`}>
              <SReview item={item} />
